...
func (c *CPS) execCmd(cmd []string) (stdO string, err error) {
	...
	set := labels.Set(map[string]string{"etcd_cluster": "galasa-ecosystem-etcd-cluster"})
	pods, err := clientSet.CoreV1().Pods(c.Cluster.Namespace).List(context.TODO(), metav1.ListOptions{LabelSelector: set.AsSelector().String()})
	if err != nil {
		return "", err
	}
	...
	req := clientSet.CoreV1().RESTClient().Post().Resource("pods").Name(pods.Items[0].Name).Namespace(c.Cluster.Namespace).SubResource("exec")
